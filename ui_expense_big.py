...
class ExpensePieBigDialog(tk.Toplevel):
    def __init__(self, dashboard, txs):
        super().__init__(dashboard)
        self.title("支出类别占比（大图）")
    ...
    def render_pie(self, total):
        ...
        w = max(w, 900)
        h = max(h, 560)
        self.canvas.create_text(16, 20, text="支出类别占比", anchor="w")
        cx = w//2
        cy = h//2 + 10
        r = int(min(w-80, h-100)/2)
        bbox = (cx-r, cy-r, cx+r, cy+r)
        palette = ["#ff6b6b","#e74c3c","#c0392b","#ff7675","#f06292","#ff8a65","#f4a261","#e9c46a","#f28b82"]
        start = 0.0
        self.id_map = {}
        ...
        for i, (name, val) in enumerate(show_items):
            ...
            self.id_map[arc] = {"name":name, "value":val, "pct":pct, "kind":"expense"}
            start += extent
    ...
    def on_click(self, e):
        ...
        BillListDialog(self.dashboard.view, initial_filters={"month": month_str, "ttype": "支出", "category": name})
    ...
    def _init_period_defaults(self):
        ...
        s = load_state()
        years = sorted({str(parse_dt(t.get("time","")).year) for t in s.get("transactions", [])})
    ...
    def _compute_items(self):
        ...
        for t in s.get("transactions", []):
            ...
            if dt.year == y and dt.month == m:
                ...
                if typ in ["支出","报销类支出"]:
                    exps[t.get("category","") or "未分类"] += float(t.get("amount",0))
        self.items = [(k, v) for k, v in exps.items() if v > 0]
